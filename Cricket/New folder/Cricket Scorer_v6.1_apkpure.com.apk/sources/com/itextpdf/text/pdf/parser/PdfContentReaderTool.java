...
public class PdfContentReaderTool {
    ...
    public static String getDictionaryDetail(PdfDictionary pdfDictionary, int i) {
        ...
        for (PdfName next : pdfDictionary.getKeys()) {
            PdfObject directObject = pdfDictionary.getDirectObject(next);
        ...
        for (PdfName pdfName : arrayList) {
            ...
            stringBuffer.append(" = ");
            stringBuffer.append(getDictionaryDetail(pdfDictionary.getAsDict(pdfName), i2));
        }
    ...
    public static String getXObjectDetail(PdfDictionary pdfDictionary) throws IOException {
        ...
        for (PdfName next : asDict.getKeys()) {
            PdfStream asStream = asDict.getAsStream(next);
            sb.append("------ " + next + " - subtype = " + asStream.get(PdfName.SUBTYPE) + " = " + asStream.getAsNumber(PdfName.LENGTH) + " bytes ------\n");
            if (!asStream.get(PdfName.SUBTYPE).equals(PdfName.IMAGE)) {
                ...
                sb.append("------ " + next + " - subtype = " + asStream.get(PdfName.SUBTYPE) + "End of Content" + "------\n");
            }
        }
        return sb.toString();
    }
