...
public class TiffWriter {
    ...
    public void writeFile(OutputStream outputStream) throws IOException {
        ...
        int ifdSize = 8 + getIfdSize();
        for (FieldBase next : this.ifd.values()) {
            int valueSize = next.getValueSize();
            if (valueSize > 4) {
                next.setOffset(ifdSize);
                ifdSize += valueSize;
            }
            next.writeField(outputStream);
        }
        writeLong(0, outputStream);
        for (FieldBase writeValue : this.ifd.values()) {
            writeValue.writeValue(outputStream);
        }
    }
