        ...
        try {
            ...
            if (indexOf >= 0) {
                ...
                if (indexOf < 0) {
                    ...
                    gregorianCalendar = new GregorianCalendar(new SimpleTimeZone(parseInt * i2 * 60000, "ZPDF"));
                }
            }
            gregorianCalendar.clear();
            int i3 = 0;
            while (true) {
                if (i >= DATE_SPACE.length) {
                    break;
                } else if (i3 >= indexOf) {
                    break;
                } else {
                    int i4 = i + 1;
                    gregorianCalendar.set(DATE_SPACE[i], Integer.parseInt(str.substring(i3, DATE_SPACE[i4] + i3)) + DATE_SPACE[i + 2]);
                    i3 += DATE_SPACE[i4];
                    i += 3;
                }
            }
