...
class PageResources {
    ...
    public void setOriginalResources(PdfDictionary pdfDictionary, int[] iArr) {
        ...
        if (pdfDictionary != null) {
            ...
            for (PdfName next : pdfDictionary.getKeys()) {
                PdfObject pdfObject = PdfReader.getPdfObject(pdfDictionary.get(next));
                if (pdfObject != null && pdfObject.isDictionary()) {
                    PdfDictionary pdfDictionary2 = (PdfDictionary) pdfObject;
                    for (PdfName add : pdfDictionary2.getKeys()) {
                        this.forbiddenNames.add(add);
                    }
                    PdfDictionary pdfDictionary3 = new PdfDictionary();
                    pdfDictionary3.merge(pdfDictionary2);
                    this.originalResources.put(next, pdfDictionary3);
                }
            }
        }
    }
    ...
    public PdfName translateName(PdfName pdfName) {
        ...
        PdfName pdfName3 = this.usedNames.get(pdfName);
        if (pdfName3 != null) {
            return pdfName3;
        }
        do {
