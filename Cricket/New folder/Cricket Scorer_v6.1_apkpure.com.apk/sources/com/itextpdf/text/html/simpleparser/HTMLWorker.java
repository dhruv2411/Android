...
public class HTMLWorker implements SimpleXMLDocHandler, DocListener {
    ...
    private static Logger LOGGER = LoggerFactory.getLogger((Class<?>) HTMLWorker.class);
    ...
    public void processLink() {
        ...
        if ((linkProcessor == null || !linkProcessor.process(this.currentParagraph, this.chain)) && (property = this.chain.getProperty(HtmlTags.HREF)) != null) {
            for (Chunk anchor : this.currentParagraph.getChunks()) {
                anchor.setAnchor(property);
            }
        }
        if (this.stack.isEmpty()) {
            this.currentParagraph = new Paragraph(new Phrase((Phrase) this.currentParagraph));
            return;
        }
        Paragraph paragraph = (Paragraph) this.stack.pop();
        paragraph.add((Element) new Phrase((Phrase) this.currentParagraph));
    ...
    public void processListItem() throws DocumentException {
        if (!this.stack.empty()) {
            ...
            if (!(pop instanceof ListItem)) {
                ...
                ((com.itextpdf.text.List) pop2).add((Element) listItem);
    ...
    public static List<Element> parseToList(Reader reader, StyleSheet styleSheet) throws IOException {
        return parseToList(reader, styleSheet, (HashMap<String, Object>) null);
    }
    ...
    public static List<Element> parseToList(Reader reader, StyleSheet styleSheet, HashMap<String, Object> hashMap) throws IOException {
        return parseToList(reader, styleSheet, (Map<String, HTMLTagProcessor>) null, hashMap);
    }
