...
public class XMPNormalizer {
            ...
            if (propertyOptions != null) {
                if (child.getOptions().isSimple()) {
                    ...
                    if (propertyOptions.isArrayAltText() && !child.getOptions().getHasLanguage()) {
                        child.addQualifier(new XMPNode(XMPConst.XML_LANG, "x-default", (PropertyOptions) null));
                    }
                } else {
    ...
    private static void transplantArrayItemAlias(Iterator it, XMPNode xMPNode, XMPNode xMPNode2) throws XMPException {
        if (xMPNode2.getOptions().isArrayAltText()) {
            ...
            xMPNode.addQualifier(new XMPNode(XMPConst.XML_LANG, "x-default", (PropertyOptions) null));
        }
        it.remove();
            ...
            if (findChildNode != null) {
                if (findChildNode.hasChildren()) {
                    ...
                    if (lookupLanguageItem < 0) {
                        xMPMeta.setLocalizedText("http://purl.org/dc/elements/1.1/", DublinCoreProperties.RIGHTS, "", "x-default", findChildNode.getChild(1).getValue(), (PropertyOptions) null);
