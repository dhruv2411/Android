...
final class TtmlNode {
    ...
    public List<Cue> getCues(long j, Map<String, TtmlStyle> map, Map<String, TtmlRegion> map2) {
        ...
        for (Map.Entry entry : treeMap.entrySet()) {
            ...
            arrayList.add(new Cue(cleanUpText((SpannableStringBuilder) entry.getValue()), (Layout.Alignment) null, ttmlRegion.line, ttmlRegion.lineType, ttmlRegion.lineAnchor, ttmlRegion.position, Integer.MIN_VALUE, ttmlRegion.width));
        }
        return arrayList;
    }
    ...
    private void traverseForStyle(Map<String, TtmlStyle> map, Map<String, SpannableStringBuilder> map2) {
        for (Map.Entry next : this.nodeEndsByRegion.entrySet()) {
            String str = (String) next.getKey();
            applyStyleToOutput(map, map2.get(str), this.nodeStartsByRegion.containsKey(str) ? this.nodeStartsByRegion.get(str).intValue() : 0, ((Integer) next.getValue()).intValue());
            for (int i = 0; i < getChildCount(); i++) {
