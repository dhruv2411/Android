...
final class zzao {
    ...
    public static MessageDigest zzlG;
    ...
    static byte[] zzb(String str, String str2, boolean z, boolean z2) {
        ...
        try {
            ...
            if (z) {
                bArr = str2.length() < 3 ? str2.getBytes("ISO-8859-1") : zzam.zza(str2, true);
            } else {
                if (str2 != null) {
                    if (str2.length() != 0) {
                        bArr = zzam.zza(zza(str2.getBytes("ISO-8859-1"), (String) null, z2), true);
                    }
                }
    ...
    private static byte[] zzb(byte[] bArr, String str, boolean z) throws NoSuchAlgorithmException, UnsupportedEncodingException {
        ...
        if (bArr.length < zza2) {
            ...
            put = ByteBuffer.allocate(zza2 + 1).put((byte) bArr.length).put(bArr).put(bArr2);
        } else {
            put = ByteBuffer.allocate(zza2 + 1).put((byte) bArr.length).put(bArr);
        }
