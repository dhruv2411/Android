...
class PinningTrustManager implements X509TrustManager {
    ...
    public PinningTrustManager(SystemKeyStore systemKeyStore2, PinningInfoProvider pinningInfoProvider) {
        ...
        String[] pins2 = pinningInfoProvider.getPins();
        for (String hexStringToByteArray : pins2) {
    ...
    private boolean isValidPin(X509Certificate x509Certificate) throws CertificateException {
        try {
            byte[] digest = MessageDigest.getInstance(SecurityConstants.SHA1).digest(x509Certificate.getPublicKey().getEncoded());
    ...
    private void checkPinTrust(X509Certificate[] x509CertificateArr) throws CertificateException {
        ...
        logger.w(Fabric.TAG, "Certificate pins are stale, (" + (System.currentTimeMillis() - this.pinCreationTimeMillis) + " millis vs " + PIN_FRESHNESS_DURATION_MILLIS + " millis) falling back to system trust.");
    }
